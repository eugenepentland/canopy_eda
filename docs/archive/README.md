...
| [`uuid-sync-research.md`](uuid-sync-research.md) | 2026-07 | How component identity worked end to end across `.sexp` ids, the `.bom` UUID and KiCad's `canopy_uuid`, and where it lost sync. | Research note. Describes the *old* three-identifier model. |
| [`uuid_research.md`](uuid_research.md) | 2026-07 | Survey of how other EDA tools solve identity drift, and the recommended path out of it. | Research note. Its recommendation (source-resident ids, sidecar stamping) is what shipped. |
| [`system_diagram_research.md`](system_diagram_research.md) | 2026-08 | Working brief for evolving the block-diagram generator from a fixed compass layout into a domain-aware pipeline. | Research note. Partly implemented. |
| [`maintainability-audit.md`](maintainability-audit.md) | 2026-07 | Survey of redundancies, duplicated helpers and lockstep-edit surfaces in `src/`. | Historical. Several items closed; file/line references are stale. |
