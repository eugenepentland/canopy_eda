...
- **Functional-view returns.** A single series R/L on the edge row of a pin group whose far end is another pin group of the same hub, on the same side and immediately adjacent, turns vertical on the pin-stub column and lands on that group's nearest stub (a pull-up from a control pin to the IC's own filtered rail, a bias inductor into the RF input). Signal nets always qualify; a supply rail only when it is private to that hub, because a shared rail's label is what lets the reader find it elsewhere. Rows are ordered so such a return sits on the edge nearest its destination, and an outside direct-return lane is pushed clear of any net label on the rows it spans. Row ORDER has one further rule that the drawing rule above does not share: a series part reaching a rail the hub itself PRODUCES — a pin named OUT / OUTS / VOUT, e.g. an LDO's PG pull-up to its own output — pairs the two rows even though the rail is shared, and the pull-up moves forward to sit immediately before the output group, which keeps its place. The output therefore stays on the output side of the box, the column split may not come between the pair, and the shared rail still gets its label. Sub-block path prefixes (`dsa/VDD_F`) are stripped before a net is classified as supply or signal.
