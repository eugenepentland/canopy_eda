...
### `(decouples …)` — required per-pin binding on a hand-named cap

A bypass cap declared as a plain `(instance …)` carries no record of *which* IC
pad it serves. On a rail that lands on ≥2 of an IC's supply pads that is
ambiguous, so a hand-named HF decoupling cap **must** declare its pin — a hard
requirement enforced as the `decoupling_unbound` ERC **error** (fails
`build`/`check`). The `decouple-unbound` layout lint on `/pcb-layout` is a
companion **warning** that additionally flags a declared binding the placement
solver couldn't resolve to its hub (e.g. a shared-plane cap paired elsewhere).

```scheme
;; bind this cap's power leg to a specific hub pad (pad number or pinout name)
(instance "C_IOVDD_3" (cap-0402 "100nF") (pin 1 "VDD3V3") (pin 2 "GND")
  (decouples "U1" 24))
;; opt out: this cap genuinely serves the whole rail (a reservoir / rail bypass)
(instance "C_VDD_BULK" (cap-0402 "1uF") (pin 1 "VDD3V3") (pin 2 "GND")
  (decouples rail))
```

Exempt from the requirement (no binding needed): bulk reservoirs ≥4.7 µF,
single-supply-pad rails (the target is unambiguous), `(decouples rail)` opt-outs,
and caps generated by a `(decouple … per-pin)` form (those already embed their
pad). Config straps tied to the rail (EN, PG, ILIM, SENSE, FB …) are excluded
from the supply-pad count, so a cap is never asked to bind to a strap.

### `(emi-couples "DOMAIN")` — intentional chassis/EMI coupling

Use this on a capacitor that intentionally provides an AC path from a named
non-supply domain, such as chassis, to a recognised ground return. ERC rejects
use on a non-capacitor, a misnamed domain, a missing ground leg, or combining it
with `(decouples …)`.

```scheme
(instance "C_CHASSIS" (cap-1812 "1nF 2kV")
  (pin 1 "CHASSIS_GND") (pin 2 "GND")
  (emi-couples "CHASSIS_GND"))
```
