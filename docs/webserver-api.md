...
- **PCB layout PNG**: `GET /api/pcb-png/:name` — server-rendered PNG of the force-directed PCB layout, so an AI agent (or any HTTP client) can *see* the board, not just parse the placement JSON. Mirrors the browser viewer's colours/projection (`src/render_pcb_png.zig` rasterizes onto `src/raster.zig`'s software canvas, encoded by `src/png.zig` — pure-Zig, no image dependency). Query: `?nets=A,B` / `?refs=U1,C3` enter *focus mode* (spotlight those nets/components in amber, dim the rest — `refs` match the bare leaf of a sub-block-prefixed ref); `?route=1` overlays routed copper + DRC markers; `?names=ref|origin|both` picks part labels (default: `origin`, the module-local sub-block-relative names, falling back to `ref` when no origin is known); `?pins=U13,C5` (or `pins=hubs`) labels those parts' pads with net names; `?crop=U1&r=6` zooms on one part (matched by ref, leaf, or origin name — combine with `?pins=`); `?sheet=1` returns a contact sheet (whole board + per-hub pin-labeled closeups); `?critique=1` draws a numbered worst-problems overlay (hottest loops by nH, longest airwire, staged parts, DRC count); `?width=`, `?layout=<saved>`, `?regen=1`, `?sub=<slug>`. When the design declares a `(board (size W H) …)` form, the physical outline rectangle + dimensions are drawn under the parts (green). Read-only; reuses the design's auto-layout cache. Repeat requests are answered from a dependency-validated in-memory image cache (`src/serve/png_cache.zig`), the picture twin of the describe cache below: the plain request and each admitted framing variant (`layout`, `width`, `names`, `nets`, `refs`, `pins`, `crop`, `r`, `cropnet`, `sheet`, `critique`, `thermal`, `scenario`, `ambient`) keep their own entry, invalidated by the design's sources, its `.layouts.json` / `.autolayout.json` / `.drc-rules.json` sidecars and its live-edit version. `?route=1` (route fresh), `?regen=1` (fresh solve), `?sub=` and every unrecognised parameter bypass it. `X-Netlisp-Png-Cache: hit|miss|bypass` reports which happened; the response still carries `Cache-Control: no-store`, because the retention that matters is the server's — a browser must not hold the picture of a board that has since been edited.
...
- **Fab outputs (full package, Gerber included)**: `GET
  ...
  the server. **A board that cannot be released is refused before it is
  built.** The gate itself is seconds of work on a large board — evaluating,
  ...
  requests/frames.
- **Complete design archive**: `POST /api/design-archive/:name` — the
  `Fabrication → Full archive` option. It uses the same saved-layout selection,
  readiness token, explicit waiver, fabrication identity, and revision lock as
...
- **Layout state**: one sidecar per design — `<design>.layouts.json` `{default, cache, layouts[]}`. `layouts[]` = named snapshots (manual saves + auto-recorded optimizer runs), `default` = the starred (★) / KiCad-sync seed, `cache` = the single-slot optimizer cache (tuning params + poses, overwritten each solve). Precedence the `/pcb-layout` viewer shows as a scorebar chip: explicit `?refine=<snapshot>` > starred (★) default > cache > fresh solve > plain grid. (The old source-authored `(placement …)` spec once sat at the top of this chain; that DSL form is retired — layout is seeded from saved snapshots / the `?rough=1` seed now, not from a spec form in the `.sexp`.) Legacy standalone `<design>.autolayout.json` is still read as a fallback and deleted on the next solve; `.placement.json` migration was dropped (all designs migrated).
  A named layout may also carry one physical heatsink assembly. In the
  PCB editor choose the ♨ tool, drag its base/contact rectangle, then select
  the physical top/bottom face and enter material, thermal-pad thickness and
  conductivity, and either a base with fin height/thickness/gap/direction or
  two solid rectangular blocks. For the two-block profile, the drawn rectangle
  and base height define the PCB-contact block; an independently sized lower
  block is centered beneath it to reach an enclosure wall. The editor derives
  a still-air theta-SA estimate, and the 3D/STEP views render the exact pad and
  metal solids,
  ...
  estimate uses a 10 W/m²K still-air film (with straight-fin efficiency for the
  finned profile); it is a comparative screening model, not enclosure or
  fan-curve CFD. In particular, a cold plate's actual case-contact resistance,
  case temperature, and case-to-air path are not inferred from geometry.
  ...
  position it or click it to edit the PCB face and outlet-to-target distance.
  ...
  When the same layout also carries a heatsink, the Thermal tab adds
  `fan_heatsink`, which solves both paths at once. A fan and heatsink on
  opposite PCB faces remain aerodynamically isolated: the fan changes only its
  own board face and the sink keeps its natural-convection theta-SA. When both
  occupy the same face, distance is measured from the fan outlet to the sink's
...
- **System thermal workspace**: `GET /systems/:name/cad` imports each board
  ...
  Both views render the exact PCB outlines, components, and heatsinks. The
  workspace imports each layout-authored fan into an independent system fan;
  fans may then be added, removed, enabled, dragged in XY, or assigned exact
  XYZ/rotation, outlet size/direction, free-air flow, static pressure, and
  installed-flow fraction values. Saving records them in the optional `fans`
  array of `netlisp-mechanical-v2`; a missing array imports the legacy
  board-attached fans, while an explicit empty array preserves the decision to
  remove every fan. Board instances can likewise be dragged in XY or edited
  numerically, with drafts isolated in browser storage and exportable through
  **Download assembly.json**. Its coupled screen reuses each board's `/api/thermal`
