...
pub const scope_form_docs = blk: {
    ...
    t[@backingInt(ScopeForm.net_class)] = .{ .scope = tl, .doc = .{
        .syntax = "(net-class \"name\" [(width MM)] [(clearance MM)] " ++
            ...
            "[(fence [(pitch MM)] [(offset MM)] [(via DIA DRILL)] [(net \"N\")])] " ++
        ...
        .summary = "Routing geometry + routing order profile and/or membership for named nets: " ++
            ...
            "implied by (max-freq …)), (offset MM) the GAP from the net's copper edge to the fence " ++
            ...
            "opts in a class with no (max-freq …). The opening is always held back from every pad " ++
            "opening by the board's mask web, so paste and solder stay dammed at QFN and passive " ++
            "lands, and an exposed stretch shorter than 1 mm keeps its mask outright — a sliver of " ++
