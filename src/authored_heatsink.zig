...
pub fn lower(placement: optimizer.Placement, spec: ?env.BoardHeatsinkSpec) ?sidecar.SavedHeatsink {
    ...
    return .{
        ...
        .profile = if (std.mem.eql(u8, sink.geometry.shape, "stepped")) .{ .stepped = .{
            .width_mm = sink.geometry.lower_width_mm,
            .length_mm = sink.geometry.lower_length_mm,
            .height_mm = sink.geometry.lower_height_mm,
        } } else .{ .finned = .{
            .height_mm = sink.geometry.fin_height_mm,
            .thickness_mm = sink.geometry.fin_thickness_mm,
            .gap_mm = sink.geometry.fin_gap_mm,
            .axis = sink.geometry.fin_axis,
        } },
...
pub fn thermalInput(
    ...
    if (saved.target_ref.len == 0) return .{
        ...
        .geometry = .{
            ...
            .profile = switch (saved.profile) {
                .finned => |fins| .{ .finned = .{
                    .height_mm = fins.height_mm,
                    .thickness_mm = fins.thickness_mm,
                    .gap_mm = fins.gap_mm,
                    .axis = std.meta.stringToEnum(scenarios.FinAxis, fins.axis) orelse .length,
                } },
