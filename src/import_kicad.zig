...
//!
//! String note: every description / value / MPN / pin name here is a slice of
//! a `.string` token this project's own tokenizer produced from the board, so
//! it is already in the grammar's ESCAPED form — it cannot contain a bare `"`
//! (that byte would have ended the token) and every `\` inside it is followed
//! by another byte of the same token. Writing such a slice straight back
//! between quotes therefore always re-parses to the exact bytes the board
//! carried, and the emitters below do exactly that. The importer used to
//! substitute `"` → `'` on the way out, which could not prevent a corruption
//! (none was reachable) and only ever mangled a well-formed `\"` into `\'` —
//! silently altering imported data. The `--title` string is the one input that
//! does NOT come through the tokenizer (it is argv / a tool argument), so it is
//! the one that is escaped, with `kicad_fmt.sexprEscape`.
...
pub const ImportError = error{ InvalidBoard, OutOfMemory, WriteFailed } || infra_fs.File.OpenError || infra_fs.File.ReadError || parser_mod.ParseError;
...
/// footprint's board rotation, so subtract it before landing the angle
/// (exact quarter-turns flatten to a width/height swap, anything else
/// keeps a pos rotation token).
...
fn emitBoardPad(
    ...
    // footprint rotation into every pad angle), then land it exactly as the
    // footprint converter does: an exact quarter-turn flattens into the
    // axis-aligned model (90/270 swap width and height, 0/180 are the
    // identity — byte-stable for every stock import), and any other angle is
    // preserved as the third (pos X Y ROT) token in netlisp's frame.
    const local = @mod(rot - fp_rot + 720.0, 360.0);
    const rot_out = footprint_conv.padRotOut(local);
    const out_sx = if (rot_out.swap) sy else sx;
    const out_sy = if (rot_out.swap) sx else sy;

    const half = padHalfExtents(out_sx, out_sy, rot_out.angle);
    min_x.* = @min(min_x.*, x - half.x);
    min_y.* = @min(min_y.*, y - half.y);
    max_x.* = @max(max_x.*, x + half.x);
    max_y.* = @max(max_y.*, y + half.y);
...
/// Half-extents of a pad's axis-aligned envelope as emitted, for the
/// courtyard accumulation. A preserved-angle pad occupies its rotated
/// rectangle — a 0.58 mm square at 45° spans 0.82 mm corner to corner — so
/// its envelope comes from the rotated corners; the axis-aligned cases keep
/// the exact half-size arithmetic (no trig, byte-stable).
fn padHalfExtents(out_sx: f64, out_sy: f64, angle_deg: f64) struct { x: f64, y: f64 } {
    if (angle_deg == 0) return .{ .x = out_sx / 2, .y = out_sy / 2 };
    const rad = angle_deg * std.math.pi / 180.0;
    const c = @abs(@cos(rad));
    const s = @abs(@sin(rad));
    return .{ .x = (out_sx * c + out_sy * s) / 2, .y = (out_sx * s + out_sy * c) / 2 };
}
...
fn buildDesignText(
    ...
    try w.print("(design-block \"{s}\"\n", .{try kicad_fmt.sexprEscape(arena, opts.title)});
...
fn emitInstance(arena: std.mem.Allocator, w: anytype, part: Part, nets: *NetNames, summary: *ImportSummary) ImportError!void {
    ...
    if (part.family) |fam| {
        ...
        try w.print("  (instance \"{s}\" ({s} \"{s}\")", .{ part.ref, fam, value });
    } else {
...
// spec: import_kicad - Preserves a pad's non-quarter-turn footprint-local angle as a netlisp-frame pos rotation token
test "footprint geometry preserves diagonal pad angles" {
    var arena_state = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena_state.deinit();
    const arena = arena_state.allocator();

    // The footprint sits at 45° on the board, so the baked pad angles are
    // local 45 (DQN0004A-MFG's diamond thermal pad shape), local 90 (an
    // exact quarter-turn), and local 100 (inside the old near-90° window
    // that flattened it to a 90° swap, 10° wrong).
    const board =
        ...
        \\    (pad "1" smd rect (at 0 0 90) (size 0.58 0.58) (net "GND"))
        \\    (pad "2" smd rect (at 3 0 135) (size 0.5 1.0) (net "A"))
        \\    (pad "3" smd rect (at -3 0 145) (size 0.9 1.8) (net "B"))))
    ;
...
test "courtyard bounds a diagonal pad by its rotated extents" {
    var arena_state = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena_state.deinit();
    const arena = arena_state.allocator();

    // A 0.58 mm square turned 45° spans 0.58·√2 ≈ 0.82 mm corner to corner:
    // half-extent 0.410, plus the 0.25 courtyard margin → ±0.660. The
    // unrotated half-size would have stopped at ±0.540, clipping the
    // diamond's corners out of the envelope.
    const board =
        \\(kicad_pcb (version 20260206)
        \\  (footprint "X:DIAMOND"
        \\    (at 0 0)
        \\    (property "Reference" "U1" (at 0 0 0))
        \\    (property "Value" "X" (at 0 0 0))
        \\    (pad "1" smd rect (at 0 0 45) (size 0.58 0.58) (net "GND"))))
    ;
    const nodes = try parser_mod.parse(arena, board);
    const parts = try parseParts(arena, nodes[0]);
    const text = try renderFootprint(arena, parts[0]);
    try testing.expect(std.mem.indexOf(u8, text, "(courtyard (rect -0.660 -0.660 0.660 0.660))") != null);
}
...
test "component naming avoids numeric atoms and family clashes" {
    ...
    defer tmp.cleanup();
    try tmp.dir.createDirPath(std.testing.io, "lib/components");
    try tmp.dir.writeFile(std.testing.io, .{ .sub_path = "lib/components/led.sexp", .data = "(component-family \"led\")" });
    const project_dir = try tmp.dir.realPathFileAlloc(std.testing.io, ".", arena);
...
// A board whose BOM text carries escaped quotes — the shape a third-party
// vendor description reaches this importer in. In a `\\` Zig literal the
// backslashes below are literal bytes, so this is exactly what the file holds.
const quoted_board =
    \\(kicad_pcb (version 20260206)
    \\  (footprint "X:ODD"
    \\    (at 0 0)
    \\    (property "Reference" "C1" (at 0 0 0))
    \\    (property "Value" "5\"ohm" (at 0 0 0))
    \\    (property "Description" "he said \"hi\"" (at 0 0 0))
    \\    (property "MPN" "P\"N" (at 0 0 0))
    \\    (pad "1" smd rect (at 0 0) (size 1 1) (net "GND") (pinfunction "IN\"A"))))
;

/// The string payload of the first `(NAME …)` child of a flat emitted form.
/// Test support for the verbatim round-trip below, which has to read a field
/// back out of the generated `.sexp` rather than trust a substring match.
fn childFormString(root: Node, name: []const u8) ?[]const u8 {
    const children = root.asList() orelse return null;
    for (children[1..]) |child| {
        if (!child.isForm(name)) continue;
        const cl = child.asList() orelse continue;
        if (cl.len >= 2) return cl[1].asString();
    }
    return null;
}

// spec: import_kicad - Writes a board's description, value, MPN and pin name back verbatim, so an escaped quote survives the import instead of being substituted
test "import writes board strings back verbatim" {
    var arena_state = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena_state.deinit();
    const arena = arena_state.allocator();

    const nodes = try parser_mod.parse(arena, quoted_board);
    const parts = try parseParts(arena, nodes[0]);
    try testing.expectEqual(@as(usize, 1), parts.len);

    const comp = CustomComp{ .name = "odd", .fp_file = "odd-fp", .part_idx = 0 };
    const text = try renderComponent(arena, comp, parts[0]);
    // The old `"` → `'` substitution turned a well-formed `\"` into `\'`.
    try testing.expect(std.mem.indexOf(u8, text, "\\'") == null);
    const want_descr =
        \\  (description "he said \"hi\"")
    ;
    try testing.expect(std.mem.indexOf(u8, text, want_descr) != null);

    // Re-parsed, the emitted fields are byte-identical to the board's own
    // tokens — the round-trip the substitution could never make.
    const comp_nodes = try parser_mod.parse(arena, text);
    try testing.expectEqualStrings(parts[0].descr, childFormString(comp_nodes[0], "description").?);
    try testing.expectEqualStrings(parts[0].mpn, childFormString(comp_nodes[0], "mpn").?);

    const pinout = try renderPinout(arena, "odd", parts[0].pads);
    const pin_nodes = try parser_mod.parse(arena, pinout);
    const pin_children = pin_nodes[0].asList().?;
    try testing.expectEqualStrings(parts[0].pads[0].func, pin_children[2].asList().?[2].asString().?);
}

// spec: import_kicad - Escapes the caller-supplied design title, the one import string that does not come through the tokenizer
test "buildDesignText escapes a raw design title" {
    var arena_state = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena_state.deinit();
    const arena = arena_state.allocator();
    ...
    const nodes = try parser_mod.parse(arena, quoted_board);
    const parts = try parseParts(arena, nodes[0]);
    parts[0].family = "cap-0402";

    var summary = ImportSummary{};
    const no_fold = import_fold.FoldResult{};
    // `--title` is argv, never a tokenizer slice: these bytes are raw.
    const text = try buildDesignText(arena, .{
        .board_path = "b.kicad_pcb",
        .project_dir = ".",
        .name = "t",
        .title = "Board \"A\" \\ rev",
    }, parts, &.{}, &no_fold, &summary);

    const want_title =
        \\(design-block "Board \"A\" \\ rev"
    ;
    try testing.expect(std.mem.indexOf(u8, text, want_title) != null);
    // The board's own value still lands verbatim beside it …
    const want_value =
        \\(instance "C1" (cap-0402 "5\"ohm")
    ;
    try testing.expect(std.mem.indexOf(u8, text, want_value) != null);
    // … and the whole design file parses, which the raw title would prevent.
