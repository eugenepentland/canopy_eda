...
/// spatial forced-air scenario; a configured fan and sink add a simultaneous
/// scenario whose forced convection remains confined to the fan's PCB face.
/// Null heatsink preserves the automatic standalone sink rung.
...
pub fn boardVerdict(ladder: Ladder) thermal.Verdict {
    ...
    return switch (row.scenario) {
        ...
        .heatsink, .fan_heatsink => .needs_heatsink,
    };
}
