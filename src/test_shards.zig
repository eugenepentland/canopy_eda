...
pub const shards: []const []const []const u8 = &.{
    ...
    &.{
        ...
        "footprint_paste.test.",
        "serve.package_store.test.",
        "serve.package_generator.test.",
        "serve.package_tools.test.",
        "serve.package_component.test.",
        ...
        "dump_args.test.",
        "test_manifest.test.",
        "serve.tool_schema.test.",
        "envelope_dump.test.",
        "bench_checkpoint.test.",
        "panelize.test.",
    ...
    &.{
        ...
        "placement.router_gap_close.test.",
    ...
    &.{
        ...
        "placement.router_maze.test.",
        "placement.router.test.a f",
        "placement.router_direct.test.",
...
// spec: Development pipeline - The Board Review Card page tests remain claimed by the shard manifest
test {
    var claims: usize = 0;
    for (shards) |shard| {
        for (shard) |filter| {
            if (std.mem.eql(u8, filter, "serve.review_card_page.test.")) claims += 1;
        }
    }
    try std.testing.expectEqual(@as(usize, 1), claims);
}
