...
const source_header_fmt =
    ";; ----------------------------------------------------------------\n" ++
    ";; READ-ONLY COPY - generated by netlisp KiCad sync (\"Push to KiCad PCB\").\n" ++
    ";; Source of truth: {s} (in the netlisp project).\n" ++
...
fn footprintNameMatches(d: *DiffContext, board_name: []const u8, short: []const u8) bool {
    ...
    // footprint it created (`netlisp-sync:c-0201`). KiCad's own footprints
...
const NetlispPad = struct { pin: []const u8, display: []const u8, full: []const u8 };

fn lessByPadNum(_: void, a: NetlispPad, b: NetlispPad) bool {
...
fn buildNetlispNetValue(
    ...
    var pads: std.ArrayList(NetlispPad) = .empty;
    var it = pad_net_map.iterator();
    while (it.next()) |entry| {
    ...
    std.mem.sort(NetlispPad, pads.items, {}, lessByPadNum);
...
fn handleInstance(d: *DiffContext, inst: export_kicad.FlatInstance, w: anytype, first: *bool) !void {
    ...
    const canopy_net = if (isPassiveRef(inst.ref_des))
        try buildNetlispNetValue(d.spc.arena, inst.ref_des, d.pad_net_map, d.pad_full_net, d.net_hub_pins)
    else
        null;
...
test "buildNetlispNetValue names the single hub pin per pad, net name otherwise" {
    // spec: serve/sync - buildNetlispNetValue renders each passive pad as destRef.destPin.net for a single hub pin, else the bare net name
    ...
    const got = try buildNetlispNetValue(arena, "C84", &pad_net_map, &pad_full_net, &net_hub_pins);
    try std.testing.expect(got != null);
    try std.testing.expectEqualStrings("U8.C3.VDD / GND", got.?);
}

test "buildNetlispNetValue lists pads in numeric order and returns null when unconnected" {
    // spec: serve/sync - buildNetlispNetValue lists a passive's pads in numeric order joined by ' / ' and returns null when the passive has no connected pads
    var arena_inst = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_inst.deinit();
    const arena = arena_inst.allocator();
    ...
    const got = try buildNetlispNetValue(arena, "R7", &pad_net_map, &pad_full_net, &net_hub_pins);
    try std.testing.expect(got != null);
    ...
    const none = try buildNetlispNetValue(arena, "C99", &pad_net_map, &pad_full_net, &net_hub_pins);
...
test "computeGroupAnchors resolves an anchor on-board through the net-signature relink tier" {
    ...
    try state.by_netsig.put(arena, "netlisp-u2", testFp("", "kid-U2", "U9"));
    var d = state.ctx(&spc, .auto);
...
// spec: serve/sync - authoritative stale pruning also removes pre-netlisp manual KiCad footprints
