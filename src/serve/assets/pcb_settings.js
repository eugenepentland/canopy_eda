...
function netClasses(){var groups=classGroups(),sync=window.PCBNetClassGeometryStatus&&window.PCBNetClassGeometryStatus(),h=title("Net classes","Search by class or net. Expand a class to inspect its effective geometry and click a member net to highlight it on the board.");
 h+='<div class="ds-class-sync"><div><strong>Routed copper</strong><span id="ds-class-sync-status">'+esc(netClassSyncSummary(sync))+'</span><small>Applies resolved widths and via sizes without moving centre lines, then refills current clearance / ground-pour gaps and reruns DRC. Current-aware power tracks are left to the DRC panel’s Widen power action. The edit is undoable.</small></div><button class="btn" id="ds-class-sync" type="button"'+(!sync||!sync.editable||!sync.changed?' disabled':'')+'>Apply classes to routed copper</button></div>';
 ...
 groups.forEach(function(g){var r=g.rows[0]||g.auth||{},source=g.sources.length?(g.sources.join(", ")):"board root",origin=g.sources.some(function(s){return !!s;})?["inherited","inherited"]:[g.auth?"authored":"resolved",g.auth?"authored":""];var search=(g.name+" "+g.nets.join(" ")).toLowerCase();
  ...
  h+='<div class="ds-metrics">'+metric("Target impedance",target)+metric("Resolved width",mm(r.width))+metric("Power branch start",Number(r.power_branch_width)>0?mm(r.power_branch_width):"—")+metric("Width source",r.width_derived?"derived from target":"authored / default")+metric("Structure",target!=="—"?impedanceStructure(r):"—")+metric("Analysis layer",target!=="—"?impedanceLayer(r):"—")+metric("Ground-pour gap",groundGap(r))+metric("Frequency band",band)+metric("Return-loss target",Number(r.max_freq_hz)>0?(rl+" dB"):"—")+metric("Clearance",mm(r.clearance))+metric("Via",mm(r.via_dia)+" / "+mm(r.via_drill))+metric("Priority",r.priority||0)+metric("Diff-pair gap",r.diff_gap>=0?mm(r.diff_gap):"—")+'</div><div class="ds-members">';
...
var DRC_HELP={
 ...
 via_current_envelope:"The same shortfall measured against the WHOLE rail because the current solve could not divide it between branches (a renamed source terminal or an incomplete consumer annotation). It is an upper bound rather than a measurement, so it warns instead of blocking, and it is raised only where the same-net barrels stitched within 2 mm cannot carry the rail between them.",
