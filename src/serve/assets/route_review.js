...
function eventText(ev){var base=labels[ev.kind]||["Router",ev.kind.replace(/_/g," "),""];if((ev.kind==="subcircuit_start"||ev.kind==="subcircuit_complete"||ev.kind==="subcircuit_failed")&&ev.detail)base=[base[0],base[1]+": "+ev.detail,base[2]];if(ev.detail==="subcircuits-only"){if(ev.kind==="initial")base=["Local stage","Subcircuit routes frozen","This run stops after validated local traces and carrier drops; no whole-board global routing is executed."];else if(ev.kind==="complete")base=["Local stage","Subcircuit-only stage complete","Boundary and deferred nets remain for a later full-board run."];}var title=base[1]+(ev.net?": "+ev.net:"");var detail=base[2];if(ev.kind==="net_failed"&&ev.net&&(data.final.search_limited||[]).indexOf(ev.net)>=0)detail="The route search reached its expansion budget. This is an algorithm/search-limit failure, not proof that no legal path exists.";if(ev.related&&ev.related.length)detail+=" Nets in this transaction: "+ev.related.join(", ")+".";if(ev.round)detail+=" Rip-up round "+ev.round+".";return [base[0],title,detail];}
function dotClass(kind){if(kind.indexOf("failed")>=0||kind==="reroute_rejected")return "fail";if(kind==="ripup"||kind==="reroute_candidate")return "rip";if(kind==="complete")return "end";if(kind.indexOf("routed")>=0||kind==="reroute_accepted"||kind==="subcircuit_complete")return "ok";return "";}
