...
(function () {
  ...
  // The WebGL scene is deliberately tessellated for display. Only geometry
  // generated by Canopy itself (board + heatsink) crosses the wire as facets;
  // the server reads each untouched library STEP and instances its original
  // analytic B-rep, sharing one definition across repeated packages.
  ...
  function exportStep() {
    ...
    // Yield once so the busy state paints before the generated bodies are
    // normalized and posted. Component triangle buffers are never serialized.
    setTimeout(function () {
      var payload;
      try { payload = { bodies: collectGeneratedStepBodies(), instances: exactStepInstances() }; }
      catch (err) {
        console.error(err);
      ...
      fetch("/api/pcb-step/" + encodeURIComponent(DATA.name), {
