...
function drawHeatsink(){var s=heatsinkRect();if(!viewSt.vis.heatsink&&!heatsinkMode&&!heatsinkDraw)return;if(!s||!(s.w>0)||!(s.h>0))return;
 ...
 if(s.shape==="stepped"){var lw=(+s.lower_width_mm||0)*S,ll=(+s.lower_length_mm||0)*S;
  if(lw>0&&ll>0)gB.appendChild(el("rect",{x:(x+(w-lw)/2).toFixed(1),y:(y+(h-ll)/2).toFixed(1),width:lw.toFixed(1),height:ll.toFixed(1),fill:"none",stroke:col,"stroke-width":1.3,opacity:.9,"stroke-dasharray":"5 3",class:"heatsink-box"}));}
 else{var axis=s.fin_axis||"length",pitch=(+s.fin_thickness_mm||1)+Math.max(+s.fin_gap_mm||0,0),across=axis==="length"?s.w:s.h,n=Math.min(512,Math.max(1,Math.floor((across+Math.max(+s.fin_gap_mm||0,0))/pitch)));
...
function ovsFp(){
 ...
 return ovsRev+"|"+s+"|"+viewSt.grid+"|"+viewSt.units+"|"+viewSt.active+"|"+viewSt.pourOp+
  ...
  "|"+(drawMode?1:0)+"|"+((drawMode&&dtrace&&dtrace.net)||"")+"|"+(viaMode?1:0)+":"+viaNet+
...
function ovsOn(w,h,kk){
 ...
 if((drawMode&&dtrace)||viaMode)return false; // live copper previews follow the cursor
...
function perimeterMaskPourNet(){if(activeLayer!==0&&activeLayer!==1)return null;
...
function paintParts(ctx,k,mov,only){
 ...
 for(var i=0;i<P.length;i++){var p=P[i];
  ...
  (p.pads||[]).forEach(function(pd){
   ...
   var hlNet=hoverNet||(drawMode&&dtrace?dtrace.net:(viaMode&&viaNet?viaNet:null)),hl=false;
...
function padPath(ctx,pd){
 ...
 else if(pd.shape==="roundrect"||pd.shape==="oval"){var hw=pd.w/2*S,hh=pd.h/2*S,
   rr=pd.shape==="oval"?Math.min(hw,hh):Math.min((+pd.rratio>0?Math.min(+pd.rratio,.5):.25)*Math.min(pd.w,pd.h)*S,hw,hh);
  ctx.moveTo(-hw+rr,-hh);ctx.lineTo(hw-rr,-hh);ctx.arcTo(hw,-hh,hw,-hh+rr,rr);
  ctx.lineTo(hw,hh-rr);ctx.arcTo(hw,hh,hw-rr,hh,rr);ctx.lineTo(-hw+rr,hh);
  ctx.arcTo(-hw,hh,-hw,hh-rr,rr);ctx.lineTo(-hw,-hh+rr);ctx.arcTo(-hw,-hh,-hw+rr,-hh,rr);ctx.closePath();}
...
function keepoutPolyPath(ctx,pts){if(!pts||pts.length<3)return false;ctx.moveTo(X(pts[0][0]),Y(pts[0][1]));
 for(var i=1;i<pts.length;i++)ctx.lineTo(X(pts[i][0]),Y(pts[i][1]));ctx.closePath();return true;}
// An authored (board … (keepout "NAME" …)) region is a SOLID rectangle and
// carries a name; the derived perimeter band is a ring pair and needs none.
// Naming it on the board is the whole point of authoring one: a reader has to
// see WHY that rectangle is empty without opening the source.
function keepoutLabel(ctx,k,q,x0,y0,x1,y1){if(!q.name)return;
 var ik=1/Math.max(k,0.01),txt=q.name+(q.side&&q.side!="both"?" · "+q.side:"");
 ctx.save();ctx.font="600 "+(10*ik).toFixed(2)+"px system-ui,sans-serif";
 ctx.textAlign="center";ctx.textBaseline="middle";ctx.fillStyle="rgba(214,178,255,0.92)";
 ctx.fillText(txt,(x0+x1)/2,(y0+y1)/2);ctx.restore();}
function paintFixedKeepouts(ctx,k){(PCB.keepouts||[]).forEach(function(q){if(!q.outer)return;
  var band=!!(q.inner&&q.inner.length>=3);
  ctx.save();ctx.beginPath();
  if(!keepoutPolyPath(ctx,q.outer)||(band&&!keepoutPolyPath(ctx,q.inner))){ctx.restore();return;}
  ...
  if(!band)keepoutLabel(ctx,k,q,x0,y0,x1,y1);});}
...
// Collapse only CONSECUTIVE coincident profile samples. Solvers and persisted
// layouts can legitimately repeat a centreline station at a section boundary;
// keeping the wider width preserves all copper while preventing a zero-length
// leg from poisoning endpoint normals, miter joins, or the fallback regions.
function rfCleanSamples(samples){var pts=[],ws=[];(samples||[]).forEach(function(s){
 var x=+(s&&s[0]),y=+(s&&s[1]),w=+(s&&s[2]);if(!isFinite(x)||!isFinite(y))return;
 if(!(w>0)||!isFinite(w))w=1e-9;var last=pts.length-1;
 if(last>=0&&Math.hypot(x-pts[last][0],y-pts[last][1])<=1e-9){ws[last]=Math.max(ws[last],w);return;}
 pts.push([x,y]);ws.push(w);});return {pts:pts,ws:ws};}
// A wide path that turns within half its width of an endpoint can fold its
// ordinary left/right offset ring across itself. Keep the compact ring in the
// common case; lower a folded ring to overlapping simple segment trapezoids
// and convex joins. A compound non-zero fill then represents their union
// without clipping legitimate copper elsewhere along a turned path.
function rfRingFolded(ring){if(!ring||ring.length<3)return true;var n=ring.length,eps=1e-9;
 function cross(a,b,c){return (b[0]-a[0])*(c[1]-a[1])-(b[1]-a[1])*(c[0]-a[0]);}
 function on(p,a,b,t){return Math.abs(cross(a,b,p))<=t&&p[0]>=Math.min(a[0],b[0])-eps&&p[0]<=Math.max(a[0],b[0])+eps&&p[1]>=Math.min(a[1],b[1])-eps&&p[1]<=Math.max(a[1],b[1])+eps;}
 function touch(a,b,c,d,t){var abc=cross(a,b,c),abd=cross(a,b,d),cda=cross(c,d,a),cdb=cross(c,d,b);
  if(((abc>t&&abd< -t)||(abc< -t&&abd>t))&&((cda>t&&cdb< -t)||(cda< -t&&cdb>t)))return true;
  return Math.abs(abc)<=t&&on(c,a,b,t)||Math.abs(abd)<=t&&on(d,a,b,t)||Math.abs(cda)<=t&&on(a,c,d,t)||Math.abs(cdb)<=t&&on(b,c,d,t);}
 for(var i=0;i<n;i++){var a=ring[i],b=ring[(i+1)%n],abx=b[0]-a[0],aby=b[1]-a[1],al=Math.hypot(abx,aby);if(al<=eps)return true;
  for(var j=i+1;j<n;j++){var c=ring[j],d=ring[(j+1)%n],cdx=d[0]-c[0],cdy=d[1]-c[1],cl=Math.hypot(cdx,cdy),t=eps*Math.max(1,al,cl),adj=(i+1)%n===j||(j+1)%n===i;
   if(adj){if(Math.abs(abx*cdy-aby*cdx)<=t&&abx*cdx+aby*cdy< -t)return true;continue;}
   if(touch(a,b,c,d,t))return true;}}
 return false;}
function rfFallbackRegions(pts,ws,ring){if(!rfRingFolded(ring))return [ring];var out=[];
 for(var i=1;i<pts.length;i++){var a=pts[i-1],b=pts[i],dx=b[0]-a[0],dy=b[1]-a[1],dl=Math.hypot(dx,dy);
  if(dl<=1e-9)continue;var nx=-dy/dl,ny=dx/dl,ah=Math.max(ws[i-1],1e-9)/2,bh=Math.max(ws[i],1e-9)/2;
  out.push([[a[0]+nx*ah,a[1]+ny*ah],[b[0]+nx*bh,b[1]+ny*bh],
   [b[0]-nx*bh,b[1]-ny*bh],[a[0]-nx*ah,a[1]-ny*ah]]);}
 for(var j=1;j+1<pts.length;j++){var p=pts[j],adx=p[0]-pts[j-1][0],ady=p[1]-pts[j-1][1],
  bdx=pts[j+1][0]-p[0],bdy=pts[j+1][1]-p[1],al=Math.hypot(adx,ady),bl=Math.hypot(bdx,bdy),h=Math.max(ws[j],1e-9)/2;
  if(al<=1e-9||bl<=1e-9)continue;
  var na=[-ady/al*h,adx/al*h],nb=[-bdy/bl*h,bdx/bl*h],cand=[[p[0]+na[0],p[1]+na[1]],[p[0]-na[0],p[1]-na[1]],
   [p[0]+nb[0],p[1]+nb[1]],[p[0]-nb[0],p[1]-nb[1]]],join=[];
  cand.forEach(function(q){if(!join.some(function(r){return Math.hypot(q[0]-r[0],q[1]-r[1])<=1e-7;}))join.push(q);});
  join.sort(function(a,b){return Math.atan2(b[1]-p[1],b[0]-p[0])-Math.atan2(a[1]-p[1],a[0]-p[0]);});
  if(join.length>=3)out.push(join);}
 return out;}
function rfCompactRing(pts,ws){if(!pts||pts.length<2)return [];var left=[],right=[];
...
function stampTitle(g,inf){var tot=(GRPS[g]||[]).length;
 ...
 return t;}
// Keep the quick button pinned to the default response even after a manual
// choice has temporarily replaced the live seed payload used by Stamp.
function defaultStampInfo(g){PCB.subseeddefaultinfo=PCB.subseeddefaultinfo||PCB.subseedinfo||{};
 return (PCB.subseeddefaultinfo||{})[g]||(PCB.subseedinfo||{})[g];}
function stampLayoutPicker(g,inf,cls){var layouts=(inf&&inf.layouts)||[];
 if(layouts.length<2)return "";
 var h='<select class="'+cls+'" data-layout-stamp="'+pEsc(g)+'" title="Stamp a specific saved layout"><option value="">Other layout…</option>';
 layouts.forEach(function(l){h+='<option value="'+pEsc(l.name)+'">'+(l.starred?'★ ':'')+pEsc(l.name)+'</option>';});
 return h+'</select>';}
function wireStampLayoutPickers(root){if(!root)return;root.querySelectorAll("[data-layout-stamp]").forEach(function(s){
 s.addEventListener("change",function(){var layout=s.value,g=s.getAttribute("data-layout-stamp");if(!layout)return;s.value="";
  if(stampGroupFn)stampGroupFn(g,layout);});});}
...
function stampBusy(g,on){document.querySelectorAll("[data-stamp],[data-grp-stamp],[data-layout-stamp],[data-save-sub],[data-grp-save]").forEach(function(b){
 if(b.getAttribute("data-stamp")!==g&&b.getAttribute("data-grp-stamp")!==g&&
    b.getAttribute("data-layout-stamp")!==g&&
...
function refreshSubcircuitData(g,layout){var url="/api/pcb-subseeds/"+encodeURIComponent(PCB.name);
 if(g&&layout)url+="?group="+encodeURIComponent(g)+"&layout="+encodeURIComponent(layout);
...
function refreshStampSeeds(g,layout){stampBusy(g,true);
 return refreshSubcircuitData(g,layout).then(function(){
   var idxs=GRPS[g]||[];
   if(!idxs.some(function(i){return !!stampSeedFor(g,P[i]);})){subPanelRefresh();
    throw new Error(layout?("saved layout ‘"+layout+"’ does not match the module’s current parts"):
     "the module has no saved layout matching its current parts");}
  });}
...
function stampGroup(g,layout){return refreshStampSeeds(g,layout).then(function(){var idxs=GRPS[g]||[],hit=[];
...
function kbdToggle(){
 ...
 kbdOv.innerHTML='<div class="kbd-box"><h3>Keyboard &amp; mouse</h3>'+
  ...
  '<div class="kbd-row"><span>Place standalone vias on a chosen net (no trace)</span><kbd>Shift+V / ⊙ Via</kbd></div>'+
...
function cloneCopper(){return {tracks:(PCB.tracks||[]).map(function(t){return {x1:t.x1,y1:t.y1,xm:t.xm,ym:t.ym,x2:t.x2,y2:t.y2,l:t.l||0,w:t.w,net:t.net||"",g:t.g,source:t.source,id:trackIdEnsure(t)};}),
 ...
 rf_paths:(PCB.rf_paths||[]).map(function(p){return {net:p.net,l:p.l||0,portal:!!p.portal,
...
function subPanelRefresh(){var box=document.getElementById("sub-panel");if(!box)return;
 ...
 var sinfo=PCB.subseeddefaultinfo||PCB.subseedinfo||{};
...
(function(){
 ...
 subPanelRefresh();
 // The page embeds the fast ★ seed. Fetch the small live payload once so the
 // adjacent picker also knows every saved layout without reloading the board.
 refreshSubcircuitData().then(function(){subPanelRefresh();if(selGroup||selRef)renderProps();}).catch(function(){});})();
}
...
function hsEstimate(s){var k=hsMaterialK(s.material),bm=s.base_mm/1000,wm=s.w/1000,lm0=s.h/1000;
 if(s.shape==="stepped"){var lw=s.lower_width_mm/1000,ll=s.lower_length_mm/1000,lh=s.lower_height_mm/1000,ov=Math.min(wm,lw)*Math.min(lm0,ll),ua=wm*lm0,la=lw*ll;
  var area=Math.max(ua-ov,0)+Math.max(la-ov,0)+la+2*(wm+lm0)*bm+2*(lw+ll)*lh;
  return {count:0,theta:1/(10*area)+bm/(k*ua)+lh/(k*ov),eta:1};}
 var axis=s.fin_axis||"length",across=(axis==="length"?s.w:s.h),along=(axis==="length"?s.h:s.w),pitch=s.fin_thickness_mm+s.fin_gap_mm;
 var n=Math.min(512,Math.max(1,Math.floor((across+s.fin_gap_mm)/pitch))),hm=s.fin_height_mm/1000,tm=s.fin_thickness_mm/1000,lm=along/1000,am=across/1000;
...
function hsFromForm(){var s={x:hsNum("hs-x-mm",0),y:hsNum("hs-y-mm",0),w:hsNum("hs-w",0),h:hsNum("hs-h",0),side:(document.getElementById("hs-side")||{}).value||"bottom",target_ref:"",material:(document.getElementById("hs-material")||{}).value||"aluminum_6063",shape:(document.getElementById("hs-shape")||{}).value||"finned",base_mm:hsNum("hs-base",2),fin_height_mm:hsNum("hs-fin-h",10),fin_thickness_mm:hsNum("hs-fin-t",1),fin_gap_mm:hsNum("hs-fin-g",1.5),fin_axis:(document.getElementById("hs-axis")||{}).value||"length",lower_width_mm:hsNum("hs-lower-w",0),lower_length_mm:hsNum("hs-lower-l",0),lower_height_mm:hsNum("hs-lower-h",0),pad_thickness_mm:hsNum("hs-pad-t",.5),pad_k_w_mk:hsNum("hs-pad-k",6)};return s;}
function hsFormValid(s){return s.w>0&&s.h>0&&s.base_mm>0&&s.pad_thickness_mm>=0&&s.pad_k_w_mk>0&&(s.shape==="stepped"?(s.lower_width_mm>0&&s.lower_length_mm>0&&s.lower_height_mm>0):(s.fin_height_mm>=0&&s.fin_thickness_mm>0&&s.fin_gap_mm>=0));}
function hsRequestedCount(){return hsNum("hs-fin-count",0);}
function hsCountFits(s){if(s.shape==="stepped")return true;var n=hsRequestedCount(),across=(s.fin_axis==="length"?s.w:s.h);return Number.isInteger(n)&&n>=1&&n<=512&&n*s.fin_thickness_mm<=across+1e-9;}
function hsCountToGap(){var s=hsFromForm();if(s.shape==="stepped"){hsResult();return;}var n=hsRequestedCount(),across=(s.fin_axis==="length"?s.w:s.h),gap=document.getElementById("hs-fin-g");
...
function hsResult(){var out=document.getElementById("hs-result"),s=hsFromForm();if(!out)return;
 ...
 var e=hsEstimate(s);out.textContent=s.shape==="stepped"?("2 solid blocks · estimated exposed-surface θSA "+e.theta.toFixed(2)+" °C/W · total height "+(s.base_mm+s.lower_height_mm).toFixed(1)+" mm"):(e.count+" fins · estimated θSA "+e.theta.toFixed(2)+" °C/W · fin efficiency "+(100*e.eta).toFixed(1)+"% · total height "+(s.base_mm+s.fin_height_mm).toFixed(1)+" mm");}
function hsProfileSync(){var stepped=((document.getElementById("hs-shape")||{}).value==="stepped");document.querySelectorAll(".hs-finned").forEach(function(e){e.hidden=stepped;});document.querySelectorAll(".hs-stepped").forEach(function(e){e.hidden=!stepped;});}
...
function hsModalOpen(rect){var m=document.getElementById("heatsink-modal");if(!m)return;var old=PCB.heatsink||{},side=old.side||(activeLayer===0?"top":"bottom");
 var vals={"hs-x-mm":rect.x,"hs-y-mm":rect.y,"hs-w":rect.w,"hs-h":rect.h,"hs-side":side,"hs-material":old.material||"aluminum_6063","hs-shape":old.shape||"finned","hs-base":old.base_mm==null ? 2 : old.base_mm,"hs-fin-h":old.fin_height_mm==null ? 10 : old.fin_height_mm,"hs-fin-t":old.fin_thickness_mm==null ? 1 : old.fin_thickness_mm,"hs-fin-g":old.fin_gap_mm==null ? 1.5 : old.fin_gap_mm,"hs-axis":old.fin_axis||"length","hs-lower-w":old.lower_width_mm>0?old.lower_width_mm:rect.w,"hs-lower-l":old.lower_length_mm>0?old.lower_length_mm:rect.h,"hs-lower-h":old.lower_height_mm>0?old.lower_height_mm:10,"hs-pad-t":old.pad_thickness_mm==null ? .5 : old.pad_thickness_mm,"hs-pad-k":old.pad_k_w_mk==null ? 6 : old.pad_k_w_mk};
...
(function(){var ids=["hs-x-mm","hs-y-mm","hs-side","hs-material","hs-base","hs-fin-h","hs-lower-w","hs-lower-l","hs-lower-h","hs-pad-t","hs-pad-k"];ids.forEach(function(id){var e=document.getElementById(id);if(e){e.addEventListener("input",hsResult);e.addEventListener("change",hsResult);}});var shape=document.getElementById("hs-shape");if(shape){shape.addEventListener("change",function(){hsProfileSync();if(shape.value==="finned")hsCountFromGap();else hsResult();});}
...
function fanResult(){var out=document.getElementById("fan-result"),s=fanFromForm();if(!out)return;if(!fanFormValid(s)){out.textContent="Enter a model, positive outlet/flow/pressure, nonnegative distance, and an installed-flow fraction from 0 to 1.";return;}
 ...
 out.textContent=(s.free_air_flow_m3_s*s.operating_flow_fraction).toFixed(4)+" m³/s installed · "+velocity.toFixed(2)+" m/s at target · ~"+pressure.toFixed(1)+" Pa audit estimate · outlet "+Math.round(area*1e6)+" mm²";}
...
svg.addEventListener("pointermove",function(ev){
 ...
 if(viaMode){viaCur=mm(ev);ovPaintSoon();return;}
...
function txArm(on){textMode=on;if(RO)textMode=false;
 ...
 if(textMode&&viaMode)viaModeSet(false);
...
function viaGeo(net){var va=parseFloat((document.getElementById("r-va")||{}).value),
 vd=parseFloat((document.getElementById("r-vd")||{}).value),c=net?netClassInfo(net):null,
 cd=c&&parseFloat(c.via_dia),cdr=c&&parseFloat(c.via_drill);
 return {dia:cd>0?cd:(va>0?va:0.4),drill:cdr>0?cdr:(vd>0?vd:0.2)};}
...
// ── Hand routing + standalone via placement ────────────────────────────
...
// A bounded, head-to-target router request supplies the optional remainder of
// the live hand route. A newer click aborts the obsolete response immediately;
// the completion endpoint itself has a short cooperative deadline.
var drawAutoTimer=null,drawAutoRequest=null,drawAutoVersion=0;
...
// Current capacity is a post-route width target, not a reason to make the pen
// head impossible to steer. An unpoured, single-ended rail is drawn at the
// board's ordinary fabrication-safe width, then locally grown after the
...
function drawNetGeometry(net){var nominal=trackW(net),target=drawPowerTarget(net),rules=PCB.rules||{};
 ...
 return {width:floor>0?floor:nominal,target:target};}
function drawWidthInit(){var s=document.getElementById("r-dw");if(!s)return;
 ...
 s.addEventListener("change",function(){try{localStorage.setItem(DRAW_W_KEY,s.value);}catch(e){}
  if(dtrace&&dtrace.n===0){var geo=drawNetGeometry(dtrace.net);dtrace.w=geo.width;dtrace.powerTarget=geo.target;}
...
drawArcInit();
// Standalone vias use the same model, net-class geometry and two-tier DRC gate
// as V during routing, but own no trace head. The status-bar net picker makes
// plane stitching possible even when the target net has no surface copper at
// the intended site; clicking a pad/track/via can pick the net instead.
function viaNetSet(net,announce){viaNet=String(net||"");var s=document.getElementById("pcb-via-net");
 if(s&&s.value!==viaNet)s.value=viaNet;viaBtnSync();ovPaintSoon();
 if(announce)routeStatMsg(viaNet?("placing vias on "+nLeaf(viaNet)+" — click the board"):"choose a via net below, or click a pad / existing copper");}
function viaControlsInit(){if((PCB.netnames||[]).length===1)viaNet=PCB.netnames[0];var s=document.getElementById("pcb-via-net");if(!s)return;
 s.textContent="";var empty=document.createElement("option");empty.value="";empty.textContent="choose…";s.appendChild(empty);
 (PCB.netnames||[]).forEach(function(net){var o=document.createElement("option");o.value=net;o.textContent=net;s.appendChild(o);});
 s.value=viaNet;
 s.addEventListener("change",function(){viaNetSet(s.value,true);});}
function viaBtnSync(){var b=document.getElementById("pcb-via"),ctl=document.getElementById("st-via");
 if(b){b.classList.toggle("on",viaMode);if(!b.classList.contains("ts-btn"))b.textContent=viaMode&&viaNet?("⊙ "+nLeaf(viaNet)):"⊙ Via";}
...
function viaModeSet(on){if(RO)return;on=!!on;
 ...
 if(on&&backingMode)backingArm(false);
 if(on&&PCB.rulerOff)PCB.rulerOff();
 viaMode=on;viaCur=null;
 if(on){drcGateInit();drcGateSessionEnsure();pcbSideTab("side-route");
  var rc=document.querySelector('.tab-chip[data-panel="panel-route"]');if(rc&&!rc.classList.contains("active"))rc.click();
  routeStatMsg(viaNet?("placing vias on "+nLeaf(viaNet)+" — click the board"):"choose a via net below, or click a pad / existing copper");}
 svg.style.cursor=on?"crosshair":"";viaBtnSync();ovPaintSoon();}
function viaPickNetAt(m){var p=padTarget(m);if(p&&p.net)return p.net;
 var v=drawHitVia(m);if(v&&v.net)return v.net;var t=drawHitTrack(m,activeLayer);if(t&&t.net)return t.net;
 var z=pourAt(m);return z&&!z.keepout&&z.net?z.net:"";}
function viaSnap(m,net){var g=snapG(),best={x:Math.round(m.x/g)*g,y:Math.round(m.y/g)*g},
 wr=9*(vb.w/Math.max(svgMetricsGet().cw,1))/S,bd=wr,key=netCollapse(net||"");
 function take(x,y){var d=Math.hypot(x-m.x,y-m.y);if(d<bd){bd=d;best={x:x,y:y,mag:true};}}
 P.forEach(function(p,i){(p.pads||[]).forEach(function(pd){if(pd.net&&netCollapse(pd.net)===key){var c=wpt(i,pd.x,pd.y);take(c.x,c.y);}});});
 (PCB.tracks||[]).forEach(function(t){if(!t.net||netCollapse(t.net)!==key)return;trackChords(t).forEach(function(c){
...
function viaPlaceAt(m){if(!viaNet){var picked=viaPickNetAt(m);if(picked){viaNetSet(picked,true);return;}
 ...
 if(viaSamePoint(q.x,q.y)){routeStatMsg("there is already a via here",true);return;}
...
function drawPadFrame(pad){if(!pad||!pad.pd)return null;
 ...
 ux/=ul;uy/=ul;var vx=-uy,vy=ux,hw=(+pd.w||0)/2,hh=(+pd.h||0)/2,radius=0,shape=pd.shape||"rect",
  portalOk=!(pd.poly&&pd.poly.length>=3)&&(shape==="rect"||shape==="roundrect"||shape==="oval");
 if(shape==="roundrect")radius=Math.min((+pd.rratio>0?Math.min(+pd.rratio,.5):.25)*Math.min(2*hw,2*hh),hw,hh);
 else if(shape==="oval")radius=Math.min(hw,hh);
...
function drawPadPortal(f,lx,ly){if(!f.portalOk)return null;var xface=Math.abs(Math.abs(lx)-f.hw)<=Math.abs(Math.abs(ly)-f.hh);
 ...
 var sy=ly<0?-1:1,hy=f.hw-f.radius;if(!(hy>1e-9)||Math.abs(lx)>hy+1e-7)return null;
...
function drawPathPadLaunch(tracks,pad,start){if(pad&&pad.via){var d=+pad.via.d||.4;return {land:d/2,span:d,portal:null};}
 ...
 for(var ti=0;ti<n;ti++){var t=start?tracks[ti]:drawReverseTrack(tracks[n-1-ti]),ch=trackChords(t),piece=trackLength(t)/ch.length;
  for(var j=0;j<ch.length;j++){var a={x:ch[j].x1,y:ch[j].y1},b={x:ch[j].x2,y:ch[j].y2},la=f.local(a),lb=f.local(b),
   ai=Math.abs(la.x)<=f.hw+1e-9&&Math.abs(la.y)<=f.hh+1e-9,bi=Math.abs(lb.x)<=f.hw+1e-9&&Math.abs(lb.y)<=f.hh+1e-9;
   if(ai&&!bi){var dx=lb.x-la.x,dy=lb.y-la.y,k=1,q;
    if(Math.abs(dx)>1e-12){q=((lb.x<0?-f.hw:f.hw)-la.x)/dx;if(q>=-1e-9&&q<=k)k=q;}
...
function drawTaperProfile(net,pad,nominal,dir){if(!pad||!pad.pd)return null;
 ...
 // a class's generic authored escape profile once the route is adaptive; that
 // older profile can be narrower than the actual pad and start outside it.
 if(powerMatch){var powerPadWidth=Math.min(+pad.pd.w||0,+pad.pd.h||0);
  if(!(powerPadWidth>0)||powerPadWidth>=nominal-1e-9)return null;
  var powerTaper=(nominal-powerPadWidth)/2;
...
// Adaptive hand-route widening. The pen leaves ordinary thin editor handles;
// these pure planners sample that ordered centreline, probe the largest clear
// width in each small interval, and turn the result into one variable-width
...
function drawAutomaticTaperPlan(tracks,startPad,endPad,nominal,taperScale){var old=(tracks||[]).slice();
 ...
 if((sr&&sr.kind==="neck")||(er&&er.kind==="neck")){
  ...
  if(old.length===1)paths=paths.concat(drawTaperPathSet(old,sr,er,nominal));
...
function drawSamePortalPath(a,b){var as=a&&a.samples||[],bs=b&&b.samples||[];
 ...
 return (close(as[0],bs[0])&&close(as[1],bs[1]))||(close(as[0],bs[1])&&close(as[1],bs[0]));}
// Saved RF paths created before pad collars already own their centreline
// tracks, so the ordinary nominal-track retrofit correctly skips them. Migrate
// those paths explicitly by deriving each pad portal from their stored samples;
// a geometry match makes this pass idempotent even though old sidecars carry no
// collar tag.
function drawRfMissingPortalGroups(){var existing=PCB.rf_paths||[],groups=[],planned=[];
 existing.forEach(function(path){if(path.portal)return;var ss=path.samples||[],c=netClassInfo(path.net||"");if(ss.length<2||!c||!(+c.impedance_ohms>0)||(+c.diff_impedance_ohms>0))return;
  var tracks=[];for(var i=1;i<ss.length;i++)tracks.push({x1:+ss[i-1][0],y1:+ss[i-1][1],x2:+ss[i][0],y2:+ss[i][1],l:path.l||0,w:+ss[i][2],net:path.net||""});
  var nominal=+c.width||+((PCB.rules||{}).track_width)||baseTrackW(),group=[],ownerIds=(path.track_ids||[]).slice();
  if(!ownerIds.length)(PCB.tracks||[]).forEach(function(t){if(rfPathOwnsTrack(path,t)){var id=trackIdEnsure(t);if(ownerIds.indexOf(id)<0)ownerIds.push(id);}});
  if(!(path.track_ids||[]).length&&ownerIds.length)path.track_ids=ownerIds.slice();
  [true,false].forEach(function(start){var q=start?ss[0]:ss[ss.length-1],pad=drawEndpointPad(path.net,path.l||0,+q[0],+q[1]);if(!pad)return;
   var launch=drawPathPadLaunch(tracks,pad,start);if(!launch||!launch.portal)return;
   var collar=drawTaperPortalPath({kind:"rf",portal:launch.portal},tracks[start?0:tracks.length-1],nominal,ownerIds);if(!collar)return;
...
function drawEnd(){var tapered={ok:true,changed:false,paths:[]},dropped=0;if(dtrace&&dtrace.n>0){tapered=drawApplyAutomaticTapers();if(!tapered.ok)return false;
 ...
 var taperMsg=tapered.power?(tapered.omitted?("power route kept at "+(+tapered.maxWidth||0).toFixed(3)+" mm — no wider DRC-clean envelope fits"):
  (tapered.changed?("power route widened locally up to "+(+tapered.maxWidth||0).toFixed(3)+" mm with compact pad tapers"):"")):
  (tapered.omitted?"automatic pad taper omitted — no DRC-clean flare fits":
...
function drawAutoAbort(){if(drawAutoRequest){drawAutoRequest.abort();drawAutoRequest=null;}}
function drawAutoReset(){drawAutoVersion++;if(drawAutoTimer){clearTimeout(drawAutoTimer);drawAutoTimer=null;}drawAutoAbort();if(dtrace)dtrace.auto=null;}
...
function drawAutoLaunch(tr,token){if(dtrace!==tr||!tr.auto||tr.auto.token!==token||tr.auto.state!=="queued")return;
 ...
 fetch("/api/pcb-route-complete/"+encodeURIComponent(PCB.name)+subq(),{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload),signal:request.signal})
  ...
  .then(function(j){if(dtrace!==tr||!tr.auto||tr.auto.token!==token)return;
   ...
   tr.auto.vias=(j.vias||[]).filter(function(v){return !v.f&&drawAutoOwn(v.net,tr);});tr.auto.rf_paths=j.rf_paths||[];tr.auto.state="ready";
   routeStatMsg("autorouter suggestion ready — Enter routes the rest");ovPaintSoon();if(tr.auto.accept)drawAutoAccept();})
  .catch(function(err){if(err&&err.name==="AbortError")return;if(dtrace===tr&&tr.auto&&tr.auto.token===token){tr.auto.state="failed";
    routeStatMsg("autorouter suggestion unavailable — keep routing by hand",true);ovPaintSoon();}})
  .then(function(){if(drawAutoRequest===request)drawAutoRequest=null;});}
function drawAutoAccept(){if(!dtrace)return false;var tr=dtrace,a=tr.auto;
 if(a&&(a.state==="queued"||a.state==="loading")){a.accept=true;routeStatMsg("autorouter finishing the remainder…");return true;}
 ...
 var shaped=tapered.power?(tapered.omitted?" · hand-routed prefix kept thin":(" · prefix widened up to "+(+tapered.maxWidth||0).toFixed(3)+" mm")):(tapered.changed?" · pad tapers added":"");
 routeStatMsg("autorouter completed the trace · "+nt+" segment"+(nt===1?"":"s")+(nv?(" · "+nv+" via"+(nv===1?"":"s")):"")+shaped+(dropped?(" · "+dropped+" RF fence via"+(dropped===1?"":"s")+" removed"):""));return true;}
...
function dpViaPair(){var pr=dtrace.pair,vg=viaGeo(dtrace.net);
...
function paintViaTool(ctx){if(!viaMode||!viaCur)return;var q=viaNet?viaSnap(viaCur,viaNet):drawSnap(viaCur),vg=viaGeo(viaNet),
 blocked=viaNet&&(viaSamePoint(q.x,q.y)||!!viaViolation(q.x,q.y,viaNet,vg.dia,vg.drill)),r=viaRenderRadius(vg.dia),rh=viaRenderRadius(vg.drill);
 ctx.save();ctx.globalAlpha=0.82;ctx.fillStyle=blocked?"#ff4d4d":((viaNet&&netColorOf(netCollapse(viaNet)))||TH.via);
...
var viaBtn=document.getElementById("pcb-via");
if(viaBtn&&!RO)viaBtn.addEventListener("click",function(){viaModeSet(!viaMode);});
...
svg.addEventListener("contextmenu",function(ev){
 ...
 if(viaMode){ev.preventDefault();viaModeSet(false);return;}
...
function traceEmPanel(o,tc){var a=traceEmInfo(o.net||"");
 ...
 var pass=!viaUnproven&&Number(a.worst_return_loss_db)>=Number(a.return_loss_target_db),local=s?n2(s.z0_ohms)+" Ω":"—",structure=s?String(s.structure||"").replace("grounded-coplanar","Grounded CPWG"):"—",pill=viaUnproven?'<span class="em-pill warn">needs 3D verification</span>':'<span class="em-pill '+(pass?'pass':'fail')+'">'+(pass?'meets':'misses')+' RL target</span>';
 var h='<section class="em-panel"><div class="em-head"><span>'+pEsc(model)+' trace analysis</span>'+pill+'</div>'+
  ...
  pRow("GND gap profile",Number(a.ground_gap_min_mm)>0?n2(a.ground_gap_min_mm)+" – "+n2(a.ground_gap_used_max_mm)+" mm":"none")+(capped?pRow("At gap cap",n2(a.ground_gap_capped_length_mm)+" mm of route"):"")+pRow("Path",n2(a.total_length_mm)+" mm · "+a.via_count+" via"+(a.via_count===1?"":"s"))+pRow("Delay",n2(a.delay_ps)+" ps")+(a.via_model_valid_to_hz==null?"":pRow("Via screen valid through",emFreq(a.via_model_valid_to_hz)))+
  ...
  (last?pRow("Zin @ "+emFreq(last.frequency_hz),n2(last.zin_re_ohms)+(last.zin_im_ohms<0?" − j":" + j")+n2(Math.abs(last.zin_im_ohms))+" Ω"):"")+'</div>'+(viaUnproven?'<div class="em-state warn">'+(a.via_model_inner_stub_unmodeled?'At least one through-via stops on an inner layer, leaving an unused barrel stub the 2.5D model cannot represent. ':'The target-synthesized via barrel is electrically long above '+pEsc(emFreq(a.via_model_valid_to_hz))+'. ')+'The plotted distributed sweep is diagnostic only and cannot establish the full-band return-loss verdict without the actual pad stack, antipads, return vias, connector launch, and 3D coupling.</div>':'')+(capped?'<div class="em-state warn">The variable GND opening reaches its '+n2(a.ground_gap_max_mm)+' mm cap here. These wider sections remain below the impedance target without a backing-plane change.</div>':'')+
  ...
  '<div class="em-limit">Uses actual routed widths, stackup, locally synthesized CPWG gap, skin effect, dielectric loss, and a passive distributed via-barrel estimate. The pour generator uses the same gap profile. Assumes FR-4 tanδ 0.02; excludes solder mask, copper roughness, radiation, connector launches, via pad stacks and stubs, return-via placement, fixed ground pads, and nearby-copper coupling. Use 3D EM or measurement for sign-off.</div></section>';return h;}
...
function renderInspProps(body){var o=insp.o,h="",hint='<div class="prop-lock">';
 if(insp.t=="track"){
  ...
  h='<div class="prop-head"><span class="prop-ref">'+pEsc(kn)+'</span><span class="prop-val">Keepout</span></div>'+
   ...
   (o.side?pRow("Side",o.side):"")+(o.reason?pRow("Reason",o.reason):"")+
   (o.poly?pRow("Vertices",String(o.poly.length)):"")+'</div>'+hint+
   (o.kind==="perimeter"?"Generated by the board perimeter-fence rule.":
    (o.kind==="board"?"Authored by (board \u2026 (keepout \u2026)) \u2014 edit the design source to move it.":
     "Imported keepout geometry is read-only in this editor."))+
   ' · <kbd>Esc</kbd> deselects</div>';
 }else{
  var dwho=drcBetween(o),dhelp=window.PCBDrcHelpForViolation?window.PCBDrcHelpForViolation(o):null;
  ...
  if(dhelp&&dhelp.description)h+='<section class="prop-drc-help"><div class="prop-drc-help-title">What this check catches</div><div class="prop-drc-help-body">'+
   (dhelp.graphic||"")+'<p>'+pEsc(dhelp.description)+'</p></div></section>';
  h+='<div class="prop-drc-occurrence"><strong>This occurrence</strong>'+pEsc(drcMsg(o))+'</div>';
 }
...
(function(){
 ...
 function apObjectRows(){return [
  ...
  {key:"heatsink",name:"Heatsink",c:"linear-gradient(90deg,#f59e0b,#38bdf8)",desc:"Physical heatsink fins or two-block cold plate; hiding it does not remove it or change thermal simulations"},
...
function apPresetApply(n){
 ...
 document.addEventListener("keydown",function(ev){
  ...
  if(ev.shiftKey)return; // Shift+V belongs to the standalone Via tool
  if(dtrace||viaMode||outlineMode||activeSketchIsArea())return;
