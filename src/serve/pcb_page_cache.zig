...
pub const Store = struct {
    ...
    /// `Entry.plain` asks the shared sweep to spend a variant first, so a burst
    /// of `?embed=1` frames cannot displace the boot-warmed design pages.
    fn trim(self: *Store, allocator: std.mem.Allocator) void {
        cache_core.evictLru(self, allocator, max_entries, max_cache_bytes);
    }

    /// Free every retained page when its owning server stops, including the
    /// keys of any render still reserved in flight.
