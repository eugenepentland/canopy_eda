//! Virtual filesystem surface for the structured CLI tools. Lets an agent treat
...
//! via the `!bool` signature so the CLI dispatcher can wrap them.
...
/// `lib/datasheets/` is read-only via CLI (PDFs are added by hand or
...
fn denialHint(rel_path: []const u8, mode: Mode) ?[]const u8 {
    ...
    if (matchesPrefix(norm, lib_datasheets) or std.mem.eql(u8, norm, "lib/datasheets")) {
        return "lib/datasheets/ is read-only from CLI — ask the user to " ++
    ...
    if (matchesPrefix(norm, lib_models) or std.mem.eql(u8, norm, "lib/models")) {
        return "lib/models/ holds 3D STEP models imported by humans; not currently writable from CLI";
    }
    if (matchesPrefix(norm, lib_parts) or std.mem.eql(u8, norm, "lib/parts")) {
        return "lib/parts/ holds curated part data imported by humans; not currently writable from CLI";
    }
    if (isReadOnlyExt(norm)) {
        if (std.mem.endsWith(u8, norm, ".layouts.json")) {
            return "PCB layout sidecars are protected; use save_pcb_layout, clear_routes, add_tracks, set_copper_zones, clean_route_topology, or restore_layout_snapshot";
        }
...
pub fn editFile(
    ...
    var new_content: std.Io.Writer.Allocating = .init(allocator);
    defer new_content.deinit();
    const cw = &new_content.writer;
    ...
    var hash_hex: [64]u8 = undefined;
    sha256Hex(new_content.written(), &hash_hex);
...
fn dirEntryKind(entry: std.Io.Dir.Entry) []const u8 {
...
pub fn listDir(
    ...
    if (list_mode == .recursive) {
        ...
        while (try walker.next()) |entry| {
            ...
            }, stat.size, @as(i64, @intCast(@divTrunc(stat.mtime.nanoseconds, std.time.ns_per_s))));
        }
    } else {
        var it = dir.iterate();
...
pub fn dirtyDesignsForPath(
    ...
    project_dir: []const u8,
    rel_path: []const u8,
) (std.mem.Allocator.Error || infra_fs.Dir.OpenError || infra_fs.Iterator.Error)![][]const u8 {
...
test "checkAcl denies writes to lib/datasheets via write_file" {
    // spec: serve/vfs - denies write_file on lib/datasheets (PDFs are read-only via CLI)
...
test "editFile applies a non-empty single-match replacement" {
    ...
    try tmp.dir.writeFile(std.testing.io, .{ .sub_path = "src/f.sexp", .data = "hello world" });
    ...
    const after = try tmp.dir.readFileAlloc(std.testing.io, "src/f.sexp", alloc, .limited64(1 << 20));
...
test "readFile errors when the offset is at or past EOF" {
    ...
    defer tmp.cleanup();
    try tmp.dir.createDirPath(std.testing.io, "src");
    try tmp.dir.writeFile(std.testing.io, .{ .sub_path = "src/f.sexp", .data = "abcde" }); // 5 bytes
    const proj = try tmp.dir.realPathFileAlloc(std.testing.io, ".", alloc);
    defer alloc.free(proj);
...
test "writeFile append concatenates onto the existing file atomically" {
    ...
    defer tmp.cleanup();
    try tmp.dir.createDirPath(std.testing.io, "src");
    try tmp.dir.writeFile(std.testing.io, .{ .sub_path = "src/f.sexp", .data = "head" });
    const proj = try tmp.dir.realPathFileAlloc(std.testing.io, ".", alloc);
    defer alloc.free(proj);
    ...
    const created = try tmp.dir.readFileAlloc(std.testing.io, "src/new.sexp", alloc, .limited64(1 << 20));
...
test "writeFile append honours the CAS guard against the pre-append file" {
    ...
    defer tmp.cleanup();
    try tmp.dir.createDirPath(std.testing.io, "src");
    try tmp.dir.writeFile(std.testing.io, .{ .sub_path = "src/f.sexp", .data = "base" });
    const proj = try tmp.dir.realPathFileAlloc(std.testing.io, ".", alloc);
    defer alloc.free(proj);
    ...
    const unchanged = try tmp.dir.readFileAlloc(std.testing.io, "src/f.sexp", alloc, .limited64(1 << 20));
