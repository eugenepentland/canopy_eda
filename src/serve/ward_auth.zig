...
//! owns every passkey, session, and OAuth grant, and
...
//! hosted route: the outer local-first facade handles the plugin-token path,
...
/// Ward roles map onto it (see `mapWardRole`). Local mode uses the same type.
...
pub const WardState = struct {
    /// Explicit hosted policy; local servers leave this false.
    enabled: bool = false,
    cfg: WardConfig = .{
        .verify_url = "",
        .login_url = "",
...
/// response (302 / 401 / 403 / 503) has already been written. Ward owns the
/// session and bearer verdicts; no locality bypass is applied in hosted mode.
...
/// Ward bearer fallback after the outer facade has checked the plugin token.
/// Returns false to continue to the session path when the bearer is invalid.
