...
const landed = @hasDecl(power_integrity, "LocalWidth");
...
pub const LocalWidth = if (landed) power_integrity.LocalWidth else struct {
    width_mm: f64,
    envelope: bool,
    reason: []const u8 = "",
};
...
pub fn targetFor(limits: Limits, local: ?LocalWidth) f64 {
    ...
    if (!(limits.branch_floor > 0)) return whole_rail;
...
/// True when some rail in scope declared `(power-branch-width MM)`, the opt-in
/// that lets a segment be sized by its own solved current.
///
/// A board without one cannot produce a single per-track target that differs
/// from its whole-rail target, so the entire apparatus — a board-wide Kirchhoff
/// solve on every route and every editor recut among it — is pure cost there.
/// Gating on this keeps such a board on the exact path, allocations included,
/// that it had before per-track targets existed.
pub fn wantsBranchSizing(limits: []const NetLimits) bool {
    for (limits) |net| {
        if (net.active and net.limits.branch_floor > 0) return true;
    }
    return false;
}
...
pub fn solveLocalWidths(
    ...
    if (comptime landed) return raw;
    return stubLocalWidths(alloc, placement, tracks, raw);
}

/// STUB — DELETE WITH `landed` ONCE `power_integrity` RETURNS `?LocalWidth`.
///
/// The `?f64` spelling cannot say whether a width came from a solved branch or
/// from the whole-rail fallback taken when the topology solve failed. Recover
/// that distinction the only way the scalar allows: a width at or above the
/// whole-rail requirement for that track's own foil is treated as an envelope.
/// The classification is safe in both directions — a genuine trunk carrying the
/// full rail current is then held at the net-wide target, which is what it
/// needs anyway.
fn stubLocalWidths(
    alloc: std.mem.Allocator,
    placement: optimizer.Placement,
    ...
    const out = try alloc.alloc(?LocalWidth, raw.len);
    for (raw, out, 0..) |width, *slot, index| {
        const mm = width orelse {
            slot.* = null;
            continue;
        };
        slot.* = .{ .width_mm = mm, .envelope = mm > 0 and mm + eps >= railWidth(placement, tracks[index]) };
    }
    return out;
}

/// STUB helper — whole-rail IPC width on one track's own foil.
fn railWidth(placement: optimizer.Placement, track: router.Track) f64 {
    if (track.net < 0) return std.math.inf(f64);
    const net_i: usize = @intCast(track.net);
    if (net_i >= placement.nets.len) return std.math.inf(f64);
    const amps = power_capacity.routingCurrentA(
        placement.rules.physical.rails,
    ...
    return power_capacity.requiredTraceWidthMm(
        amps,
        placement.rules.physical.stack.foilMm(physical),
        outer,
    ) orelse std.math.inf(f64);
}
...
test "per-track power target follows the solved branch current" {
    ...
    // A class that never declared `(power-branch-width MM)` did not opt into
    // branch sizing, so every segment keeps the authored whole-rail width even
    // where the solve proves the branch carries nothing.
