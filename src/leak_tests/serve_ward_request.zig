...
//! explicit hosted mode, session redirects, scope/role gating,
...
/// that stay on the unconfigured path skip it. `deinit` reclaims
...
const TestEnv = struct {
    a: std.mem.Allocator,
    state: serve.ServerState = .{ .ward = .{ .enabled = true } },
...
test "ward-request: an unauthenticated api request gets 401 json not a redirect" {
...
test "ward-request: an unauthenticated page request gets a 302 to the ward login" {
    var env = TestEnv{ .a = std.testing.allocator };
    defer env.deinit();
    env.initWard("http" ++ "://v", login_url, "http" ++ "://i");
    var ht = httpz.testing.init(.{});
...
test "ward-request: session write-gate forbids a reader mutation and admits the rest" {
    var env = TestEnv{ .a = std.testing.allocator };
    defer env.deinit();
    env.initWard("http" ++ "://v", login_url, "http" ++ "://i");
...
test "ward-request: the write-gate exempts pcb-drc compute but not the pcb-drc-rules mutation" {
    var env = TestEnv{ .a = std.testing.allocator };
    defer env.deinit();
    env.initWard("http" ++ "://v", login_url, "http" ++ "://i");
    const now = clock.timestamp();
...
test "ward-request: a plugin token wins the sync path and a bogus one falls through" {
...
test "ward-request: a cache-seeded ward bearer admits the sync path as the fallback" {
    var env = TestEnv{ .a = std.testing.allocator };
    defer env.deinit();
    env.initWard("http" ++ "://v", login_url, "http" ++ "://i");
...
test "ward-request: the sync bearer fallback is role-gated, not scope-only" {
    ...
    for (rows) |row| {
        ...
        ht.header("authorization", "Bearer " ++ bearer_token);
        var srv = env.server(false); // no plugin token matches (empty store)
        try std.testing.expectEqual(row.admitted, try auth.authMiddleware(&srv, ht.req, ht.res));
...
test "ward-request: a foreign-scoped ward bearer does not admit the sync path" {
    var env = TestEnv{ .a = std.testing.allocator };
    defer env.deinit();
    env.initWard("http" ++ "://v", login_url, "http" ++ "://i");
...
test "ward-request: a malformed sync bearer with ward configured is not admitted" {
    var env = TestEnv{ .a = std.testing.allocator };
    defer env.deinit();
    env.initWard("http" ++ "://v", login_url, "http" ++ "://i");
    var ht = httpz.testing.init(.{});
...
test "ward-request: oom during the sync bearer fallback errors instead of admitting" {
    var env = TestEnv{ .a = std.testing.allocator };
    defer env.deinit();
    env.initWard("http" ++ "://v", login_url, "http" ++ "://i");
    ...
    var srv = env.server(false);
    try std.testing.expectError(error.OutOfMemory, auth.authMiddleware(&srv, ht.req, ht.res));
}

// ── RFC 9728 protected-resource metadata ─────────────────────────────────────

// spec: serve - The protected-resource metadata derives its resource url from the host and names the ward server
test "ward-request: protected-resource metadata derives host and names the ward server" {
    var env = TestEnv{ .a = std.testing.allocator };
    defer env.deinit();
    env.state.ward.cfg = .{
        .verify_url = "",
...
// spec: serve - Ward hosting fails closed without configuration even for loopback or allow-remote requests
test "ward-request: local peer and remote override never bypass Ward" {
    for ([_]bool{ false, true }) |allow_remote| {
        var env = TestEnv{ .a = std.testing.allocator };
        defer env.deinit();
        var ht = httpz.testing.init(.{});
        defer ht.deinit();
        ht.url("/");
        var srv = env.server(allow_remote);
        try std.testing.expect(!try auth.authMiddleware(&srv, ht.req, ht.res));
        try std.testing.expectEqual(@as(u16, 503), ht.res.status);
...
// spec: serve - Ward hosting serves health probes and metadata before verification
test "ward-request: public health and metadata are dispatched before verification" {
    var env = TestEnv{ .a = std.testing.allocator };
    defer env.deinit();
    var ht = httpz.testing.init(.{});
    defer ht.deinit();
    ...
    try expectContains(ht.res.body, "authorization_servers");
}
