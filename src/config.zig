...
/// True when `NETLISP_ALLOW_REMOTE` is set (env or `.env`) to a non-empty
/// value — the environment spelling of `netlisp serve --allow-remote`, which
/// treats every request as an admin because an authenticating reverse proxy is
/// in front of the server. The env read lives here so the `ban-env` policy
/// holds (config.zig is the one module that touches the environment; callers
/// receive a plain bool).
pub fn allowRemote(allocator: std.mem.Allocator) bool {
    const v = lookup(allocator, "NETLISP_ALLOW_REMOTE") orelse return false;
    allocator.free(v);
    return true;
}
