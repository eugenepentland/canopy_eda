...
const TestPorts = struct {
    default: u16 = 0,
    dispatch_default: u16 = 0,
    dispatch: u16 = 0,
    dispatch_context: u16 = 0,
    reuse: u16 = 0,
    handle: u16 = 0,
    websocket: u16 = 0,
    cors_wildcard: u16 = 0,
...
test "tests:beforeAll" {
...

    {
        ...
        test_ports.default = try boundPort(&default_server);
    }

    {
        dispatch_default_server = try Server(*TestHandlerDefaultDispatch).init(t.io, ga, .{ .address = .localhost(0) }, &test_handler_default_dispatch1);
        ...
        test_ports.dispatch_default = try boundPort(&dispatch_default_server);
    }

    {
        dispatch_server = try Server(*TestHandlerDispatch).init(t.io, ga, .{ .address = .localhost(0) }, &test_handler_dispatch);
        ...
        test_ports.dispatch = try boundPort(&dispatch_server);
    }

    {
        dispatch_action_context_server = try Server(*TestHandlerDispatchContext).init(t.io, ga, .{ .address = .localhost(0) }, &test_handler_disaptch_context);
        ...
        test_ports.dispatch_context = try boundPort(&dispatch_action_context_server);
    }

    {
        // with only 1 worker, and a min/max conn of 1, each request should
        // hit our reset path.
        reuse_server = try Server(void).init(t.io, ga, .{ .address = .localhost(0), .workers = .{ .count = 1, .min_conn = 1, .max_conn = 1 } }, {});
        ...
        test_ports.reuse = try boundPort(&reuse_server);
    }

    {
        handle_server = try Server(TestHandlerHandle).init(t.io, ga, .{ .address = .localhost(0) }, TestHandlerHandle{});
        test_server_threads[5] = try handle_server.listenInNewThread();
        test_ports.handle = try boundPort(&handle_server);
    }

    {
        websocket_server = try Server(TestWebsocketHandler).init(t.io, ga, .{ .address = .localhost(0) }, TestWebsocketHandler{});
        ...
        test_ports.websocket = try boundPort(&websocket_server);
    }

    {
        cors_wildcard_server = try Server(void).init(t.io, ga, .{ .address = .localhost(0) }, {});
        ...
        test_ports.cors_wildcard = try boundPort(&cors_wildcard_server);
    }

    {
        cors_single_server = try Server(void).init(t.io, ga, .{ .address = .localhost(0) }, {});
        ...
        test_ports.cors_single = try boundPort(&cors_single_server);
    }

    {
        cors_multiple_server = try Server(void).init(t.io, ga, .{ .address = .localhost(0) }, {});
        ...
        test_ports.cors_multiple = try boundPort(&cors_multiple_server);
    }

    {
        ...
        test_ports.chunked = try boundPort(&chunked_server);
    }
...
test "httpz: quick shutdown" {
    var server = try Server(void).init(t.io, t.allocator, .{ .address = .localhost(0) }, {});
...
test "httpz: bind failure releases mutex" {
    ...
    var server1 = try Server(void).init(t.io, t.allocator, .{ .address = .localhost(0) }, {});
    const thrd1 = try server1.listenInNewThread();
    const port = try boundPort(&server1);
    defer {
        server1.stop();
        thrd1.join();
        server1.deinit();
    ...
    var server2 = try Server(void).init(t.io, t.allocator, .{ .address = .localhost(port) }, {});
...
test "httpz: invalid request" {
    const stream = testStream(test_ports.default);
...
test "httpz: body too big" {
    const stream = testStream(test_ports.dispatch_default);
    defer stream.close(t.io);
    var writer = stream.writer(t.io, &.{});
    const w = &writer.interface;
...
test "httpz: chunked body" {
    ...
    const stream = testStream(test_ports.chunked);
    defer stream.close(t.io);
    var writer = stream.writer(t.io, &.{});
    const w = &writer.interface;
...
test "httpz: CORS wildcard origin" {
    const stream = testStream(test_ports.cors_wildcard);
    defer stream.close(t.io);
...
test "httpz: CORS single origin" {
    const stream = testStream(test_ports.cors_single);
    defer stream.close(t.io);
...
test "httpz: CORS multiple origins" {
    const stream = testStream(test_ports.cors_multiple);
    defer stream.close(t.io);
...
test "httpz: writer re-use" {
    ...
    const stream = testStream(test_ports.reuse);
    defer stream.close(t.io);
    var writer = stream.writer(t.io, &.{});
    const w = &writer.interface;
...
test "httpz: custom dispatch without action context" {
    const stream = testStream(test_ports.dispatch);
    defer stream.close(t.io);
    var writer = stream.writer(t.io, &.{});
    const w = &writer.interface;
    try w.writeAll("GET / HTTP/1.1\r\nContent-Length: 0\r\n\r\n");
...
test "httpz: custom dispatch with action context" {
    const stream = testStream(test_ports.dispatch_context);
    defer stream.close(t.io);
    var writer = stream.writer(t.io, &.{});
    const w = &writer.interface;
...
test "httpz: custom handle" {
    const stream = testStream(test_ports.handle);
    defer stream.close(t.io);
    var writer = stream.writer(t.io, &.{});
    const w = &writer.interface;
...
test "websocket: invalid request" {
    const stream = testStream(test_ports.websocket);
    defer stream.close(t.io);
    var writer = stream.writer(t.io, &.{});
    const w = &writer.interface;
...
test "websocket: stress" {
    ...
    // Start a dedicated ephemeral listener in that case.
    var stress_server: ?Server(TestWebsocketHandler) = null;
    var stress_listen_thread: ?Thread = null;
    var port = test_ports.websocket;
    if (port == 0) {
        stress_server = try Server(TestWebsocketHandler).init(t.io, t.allocator, .{ .address = .localhost(0) }, TestWebsocketHandler{});
        var router = try stress_server.?.router(.{});
        router.get("/ws", TestWebsocketHandler.upgrade, .{});
        stress_listen_thread = try stress_server.?.listenInNewThread();
        port = try boundPort(&stress_server.?);
        try testing.waitForPort(port);
    }
    ...
    for (&threads) |*thread| {
        thread.* = Thread.spawn(.{}, struct {
            fn run(server_port: u16) void {
                const stream = testStream(server_port);
                defer stream.close(t.io);
...
fn boundPort(server: anytype) !u16 {
    const listener = server._listener orelse return error.ServerNotListening;
    var address = try posix.Address.initIp4(.{ 0, 0, 0, 0 }, 0);
